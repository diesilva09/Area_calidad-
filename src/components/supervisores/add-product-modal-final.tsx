...
import { productoPesosConfigService } from '@/lib/producto-pesos-config-service';
import { temperaturaEquiposService } from '@/lib/temperatura-equipos-service-enhanced';
...
export function AddEditProductModalEnhanced({
  isOpen,
  onOpenChange,
  onSave,
  categories,
  ...
  async function onSubmit(values: AddItemFormValues) {
    try {
      ...
      // Guardar pesos configurados si existen
      if (values.pesosConfigs && values.pesosConfigs.length > 0 && values.id) {
        for (const pesoConfig of values.pesosConfigs) {
          if (pesoConfig.envase_tipo) {
            await productoPesosConfigService.create({
      ...
      // Guardar temperaturas configuradas si existen
      if (values.temperaturas && values.temperaturas.length > 0) {
        for (const temp of values.temperaturas) {
          if (temp.horario && temp.realizado_por) {
            // Convertir fecha actual a formato serial de Excel
            const fechaExcel = Math.floor((new Date().getTime() - new Date('1900-01-01').getTime()) / (1000 * 60 * 60 * 24)) + 25569;
            
            await temperaturaEquiposService.create({
              fecha: fechaExcel,
              ...temp
            });
          }
        ...
        <Form {...form}>
          ...
          >
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={isCodeDuplicate || isSubmitting}>
