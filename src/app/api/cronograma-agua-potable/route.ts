...
export async function GET(request: NextRequest) {
  try {
    ...
    if (id) {
      const result = await pool.query(
        `SELECT * FROM lab_microbiologia.cronograma_agua_potable WHERE id = $1`,
        [id]
      );
    ...
    const result = await pool.query(
      `SELECT DISTINCT ON (cap.id)
        cap.*,
        cm.codigo as codigo_muestra
      FROM lab_microbiologia.cronograma_agua_potable cap
      LEFT JOIN lab_microbiologia.custodia_muestras cm 
        ON cm.cronograma_task_id = cap.id
      ORDER BY cap.id, cap.fecha_programada DESC`
    );

    return NextResponse.json(result.rows);
  } catch (error) {
    console.error('Error al obtener tareas de agua potable:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!area || !ubicacion || !fecha_programada) {
      return NextResponse.json(
        { error: 'Faltan campos obligatorios: area, ubicacion, fecha_programada' },
        { status: 400 }
      );
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.log('🔍 PUT cronograma-agua-potable - ID:', id, 'Body:', body);
