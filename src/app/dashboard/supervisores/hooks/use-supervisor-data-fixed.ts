...
export interface SupervisorHandlers {
  ...
  // Limpieza Tasks
  handleCreateLimpiezaTask: (task: Omit<LimpiezaTask, 'id' | 'created_at' | 'updated_at'>) => Promise<boolean>;
...
export function useSupervisorData() {
  ...
  // Category handlers
  const handleCreateCategory = useCallback(async (category: Omit<ProductCategory, 'id'>): Promise<boolean> => {
    try {
      // Validate data
      ...
      console.log('✅ Create Category completed successfully');
      return true;
  ...
  }, [loadData, toast, validateCategory]);

  const handleEditCategory = useCallback(async (category: ProductCategory): Promise<boolean> => {
    try {
      // Validate data
      const validationErrors = validateCategory(category);
      if (validationErrors.length > 0) {
      ...
      console.log('✅ Edit Category completed successfully');
      return true;
  ...
  // Product handlers
  const handleCreateProduct = useCallback(async (product: Omit<Product, 'id'>): Promise<boolean> => {
    try {
      // Validate data
  ...
  }, [loadData, toast, validateProduct]);

  const handleEditProduct = useCallback(async (product: Product): Promise<boolean> => {
    try {
      // Validate data (convert to Omit type for validation)
      ...
      console.log('✅ Edit Product completed successfully');
      return true;
