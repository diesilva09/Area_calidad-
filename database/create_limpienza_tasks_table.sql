...
CREATE TABLE IF NOT EXISTS limpienza_tasks (
...
-- Crear índices
CREATE INDEX IF NOT EXISTS idx_limpienza_fecha ON limpienza_tasks(fecha);
CREATE INDEX IF NOT EXISTS idx_limpienza_status ON limpienza_tasks(status);
CREATE INDEX IF NOT EXISTS idx_limpienza_area ON limpienza_tasks(area);
...
CREATE TRIGGER update_limpienza_tasks_updated_at 
    BEFORE UPDATE ON limpienza_tasks 
    FOR EACH ROW 
    EXECUTE FUNCTION update_updated_at_column();

-- Insertar algunas labores de ejemplo para pruebas
INSERT INTO limpienza_tasks (area, tipo_muestra, detalles, fecha, status, created_by) VALUES
